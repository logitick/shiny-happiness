...
public class Programmer {
  ...
  private ArrayList<File> traverseProject(File start, ArrayList<File> out) {
    if (start.isDirectory()) {
      for (File subFile : start.listFiles(project.getType().getFileFilter())) {
        if (subFile.isDirectory()) {
          traverseProject(subFile, out);
        } else {
          out.add(subFile);
        }
      }
